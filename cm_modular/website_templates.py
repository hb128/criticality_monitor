...
def render_enhanced_html(
    ...
    (function() {
        ...
        if (DISABLED) { console.log('[auto-reload] disabled via query param'); return; }
        ...
        async function headProbe(url) {
            try {
                ...
                return sig;
            } catch (e) { consecutiveHeadFailures += 1; return null; }
        }
        ...
        async function probe() {
            ...
            if (!sig && consecutiveHeadFailures >= 2) { sig = await fullProbe(cacheBustUrl); }
            if (sig) {
                ...
                if (!baselineSignature) { baselineSignature = sig; console.log('[auto-reload] baseline set'); }
            } else { console.log('[auto-reload] probe failed'); }
